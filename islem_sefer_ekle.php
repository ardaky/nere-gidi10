...
try {
    $sefer_uuid = uuid_olustur();

    $sql = "INSERT INTO Trips (uuid, company_id, departure_city, destination_city, departure_time, arrival_time, price, capacity)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($sonuc) {
        $_SESSION['basari_mesaji'] = "Yeni sefer başarıyla eklendi.";
        header("Location: firma_admin_panel.php");
        exit;
    } else {
        throw new Exception("Sefer eklenirken bilinmeyen bir veritabanı hatası oluştu.");
    }
